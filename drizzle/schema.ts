...
export const users = mysqlTable("users", {
  ...
  password: varchar("password", { length: 255 }), // Hash bcrypt da senha (null para OAuth)
...
export const config = mysqlTable("config", {
  ...
  // Configurações do TTLFilter (Time-To-Live Filter - Janela Operacional)
  // IMPORTANTE: O TTL avalia tempo restante até o MINUTO 45 (limite de entrada), NÃO até o minuto 60!
  // Janela operacional M60: 35-45min (10 minutos disponíveis)
  ttlEnabled: boolean("ttlEnabled").default(false).notNull(), // Habilitar filtro TTL (DESATIVADO por padrão)
  ttlMinimumSeconds: int("ttlMinimumSeconds").default(180).notNull(), // Tempo mínimo restante na janela operacional (padrão: 180s = 3min)
  ttlTriggerDelayBuffer: int("ttlTriggerDelayBuffer").default(120).notNull(), // Buffer para atraso do gatilho (padrão: 120s = 2min)
